...
public final class Crc32c {
    ...
    public static String hashToBase64EncodedString(String input){
        try {
            return hashToBase64EncodedString(input, "UTF-8");
        }catch (UnsupportedEncodingException e){
            throw new RuntimeException("UTF-8 unsupported should never occur", e);
        }
    }

    public static String hashToBase64EncodedString(String input, String charset)throws UnsupportedEncodingException{
        byte[] bytes = input.getBytes(charset);
        Crc32c instance = new Crc32c();
        instance.update(bytes, 0, bytes.length);
        bytes = instance.getValueAsBytes();
